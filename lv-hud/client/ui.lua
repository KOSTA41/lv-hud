    ...
    while true do 
	 local wait = 1000
        if QBCore ~= nil and isLoggedIn and QBHud.Show then
			...
			local stamina = GetPlayerSprintTimeRemaining(PlayerId()) * 3.6
            SendNUIMessage({
                action = "hudtick",
        ...
        if QBCore ~= nil and isLoggedIn and QBHud.Show then
            if IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if speed >= QBStress.MinimumSpeed then
                    TriggerServerEvent('lv-hud:Server:GainStress', math.random(1, 2))
                end
            end
        end
        Citizen.Wait(wait)
...
RegisterNetEvent("hud:client:UpdateStamina")
AddEventHandler("hud:client:UpdateStamina", function(stamina)
    stamina = stamina
end)
...
RegisterNetEvent('lv-hud:client:ToggleHarness')
AddEventHandler('lv-hud:client:ToggleHarness', function(toggle)
    SendNUIMessage({
        action = "harness",
        toggle = toggle
    })
...
RegisterNetEvent('lv-hud:client:UpdateNitrous')
AddEventHandler('lv-hud:client:UpdateNitrous', function(toggle, level, IsActive)
    SendNUIMessage({
        action = "nitrous",
        toggle = toggle,
        level = level,
...
RegisterNetEvent('lv-hud:client:UpdateDrivingMeters')
AddEventHandler('lv-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
...
RegisterNetEvent('lv-hud:client:UpdateVoiceProximity')
AddEventHandler('lv-hud:client:UpdateVoiceProximity', function(Proximity)
    SendNUIMessage({
        action = "proximity",
        prox = Proximity
    })
...
RegisterNetEvent('lv-hud:client:ProximityActive')
AddEventHandler('lv-hud:client:ProximityActive', function(active)
    SendNUIMessage({
        action = "talking",
        IsTalking = active
    })
